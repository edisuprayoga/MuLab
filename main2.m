function status = main2(opt_pos,opt_spn,opt_job,muon,coord,R_dip,file1,file2,ion_no, ...
...
for i = 1:opt;
    ...
    if opt_pos == 1 % perfect
        if opt_spn == 1 % manual
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip111(muon(i,:),coord,R_dip,[file1 num2str(i)],ion_no,mag1,spin1,gpu,log); 
                else Hdip = dip111(muon(i,:),coord,R_dip,file1,ion_no,mag1,spin1,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip112(muon(i,:),coord,R_dip,[file1 num2str(i)],ion_no,mag1,spin1,zpe_range,zpe_ngrid,gpu,log);
                else Hdip = dip112(muon(i,:),coord,R_dip,file1,ion_no,mag1,spin1,zpe_range,zpe_ngrid,gpu,log); end
        ...
        elseif opt_spn == 2 % vasp
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip121(muon(i,:),coord,R_dip,[file1 num2str(i)],lim,gpu,log);
                else Hdip = dip121(muon(i,:),coord,R_dip,file1,lim,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip122(muon(i,:),coord,R_dip,[file1 num2str(i)],zpe_range,zpe_ngrid,lim,gpu,log);
                else Hdip = dip122(muon(i,:),coord,R_dip,file1,zpe_range,zpe_ngrid,lim,gpu,log); end
            elseif opt_job == 3 % test
        ...
        elseif opt_spn == 3 % chgcar
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip131(muon(i,:),coord,R_dip,[file1 num2str(i) '/CHGCAR'],gpu,log);
                else Hdip = dip131(muon(i,:),coord,R_dip,file1,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip132(muon(i,:),coord,R_dip,[file1 num2str(i) '/CHGCAR'],zpe_range,zpe_ngrid,gpu,log);
                else Hdip = dip132(muon(i,:),coord,R_dip,file1,zpe_range,zpe_ngrid,gpu,log); end
            elseif opt_job == 3 % test
    ...
    elseif opt_pos == 2 % defect
        if opt_spn == 1 % manual
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip211(muon(i,:),coord,R_dip,[file1 num2str(i)],file2,ion_no,mag1,spin1,mag2,spin2,gpu,log);
                else Hdip = dip211(muon(i,:),coord,R_dip,file1,file2,ion_no,mag1,spin1,mag2,spin2,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip212(muon(i,:),coord,R_dip,[file1 num2str(i)],file2,ion_no,mag1,spin1,mag2,spin2,zpe_range,zpe_ngrid,gpu,log); 
                else Hdip = dip212(muon(i,:),coord,R_dip,file1,file2,ion_no,mag1,spin1,mag2,spin2,zpe_range,zpe_ngrid,gpu,log); end
            elseif opt_job == 3 % test
        ...
        elseif opt_spn == 2 % vasp
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip221(muon(i,:),coord,R_dip,[file1 num2str(i)],file2,lim,gpu,log);
                else Hdip = dip221(muon(i,:),coord,R_dip,file1,file2,lim,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip222(muon(i,:),coord,R_dip,[file1 num2str(i)],file2,zpe_range,zpe_ngrid,lim,gpu,log);
                else Hdip = dip222(muon(i,:),coord,R_dip,file1,file2,zpe_range,zpe_ngrid,lim,gpu,log); end
            elseif opt_job == 3 % test
        ...
        elseif opt_spn == 3 % chgcar
            if opt_job == 1 % dipole
                if opt > 1; Hdip = dip231(muon(i,:),coord,R_dip,[file1 num2str(i) '/CHGCAR'],file2,gpu,log);
                else Hdip = dip231(muon(i,:),coord,R_dip,file1,file2,gpu,log); end
            elseif opt_job == 2 % zpe
                if opt > 1; Hdip = dip232(muon(i,:),coord,R_dip,[file1 num2str(i) '/CHGCAR'],file2,zpe_range,zpe_ngrid,gpu,log);
                else Hdip = dip232(muon(i,:),coord,R_dip,file1,file2,zpe_range,zpe_ngrid,gpu,log); end
            elseif opt_job == 3 % test
...
if opt > 1; pathname = pwd; fprintf('\n\n>> Writing log file\n');
    ...
    if opt_spn == 1; if opt > 1; geo = poscar([file1 num2str(1)]); else geo = poscar(file1); end
    ...
    elseif opt_spn == 2
        if opt > 1; spin1 = outcar([file1 num2str(1) '/OUTCAR']); else spin1 = outcar([file1 '/OUTCAR']); end
        num = (1:size(spin1,1))';
    ...
    elseif opt_spn == 3;
        if opt > 1; [~,Sx2,Sy2,Sz2,geo2] = chgcar([file1 num2str(1) '/CHGCAR']); else [~,Sx2,Sy2,Sz2,geo2] = chgcar(file1); end
