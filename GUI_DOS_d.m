function varargout = GUI_DOS_d(varargin)
% GUI_DOS_d MATLAB code by Edi Suprayoga
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_DOS_d_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_DOS_d_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function GUI_DOS_d_OpeningFcn(hObject, ~, handles, varargin)
handles.output = hObject; guidata(hObject, handles);
set(handles.figure1,'Color',[0.8 0.8 0.8])

function varargout = GUI_DOS_d_OutputFcn(~, ~, handles) 
varargout{1} = handles.output; set(handles.autorized,'Value',1);

function cek_reverse_Callback(~, ~, handles)
if get(handles.cek_reverse,'Value') == 1
    set(handles.cek_reverse,'ForegroundColor',[0 0 1])
else set(handles.cek_reverse,'ForegroundColor',[0 0 0])
end
...
lwidth = str2double(get(handles.disp_width,'String'));
Msize = str2double(get(handles.disp_msize,'String'));
pathname = get(handles.disp_pathname,'String'); pathname = [pathname '/'];
opt_dos = get(handles.opt_dos,'Value');
geo = poscar(pathname);
[en, dos, ef, pdos] = doscar(pathname);
[eig,k,~] = eigenval(pathname); 
...
elseif get(handles.opt_no,'Value') == 1 && get(handles.cek_all,'Value') == 1 
...
else dz2_up = 0; dz2_dn = 0; dx2y2_up = 0; dx2y2_dn = 0;
    if size(pdos,2) < 33 % spin collinear
        if (get(handles.dxy,'Value') == 1); j1 = [j1 9];  j2 = [j2 10]; j0 = [j0 5]; end
        if (get(handles.dyz,'Value') == 1); j1 = [j1 11]; j2 = [j2 12]; j0 = [j0 6]; end
        if (get(handles.dxz,'Value') == 1); j1 = [j1 15]; j2 = [j2 16]; j0 = [j0 8]; end
        if size(dos,2) == 1 % para
            for i = ion
                dx2y2_up = dx2y2_up + pdos(:,9,i); dz2_up = dz2_up + pdos(:,7,i);
            end
        else % Spin 
            for i = ion
                dx2y2_up = dx2y2_up + pdos(:,17,i); dz2_up = dz2_up + pdos(:,13,i);
                dx2y2_dn = dx2y2_dn + pdos(:,18,i); dz2_dn = dz2_dn + pdos(:,14,i);
            end
        end
        j01 = j0;
    else % spin non-collinear
        if get(handles.dxy,'Value') == 1; j01 = [j01 17]; j0 = [j0 5]; end
        if get(handles.dyz,'Value') == 1; j01 = [j01 21]; j0 = [j0 6]; end
        if get(handles.dxz,'Value') == 1; j01 = [j01 29]; j0 = [j0 8]; end
        for i = ion
            dx2y2_up = dx2y2_up + pdos(:,33,i); dz2_up = dz2_up + pdos(:,25,i);
        end
    end
    ...
    if (get(handles.dx2y2,'Value') == 1); j0 = [j0 9];
        y1 = y1 + dx2y2_up; y2 = y2 + dx2y2_dn;  
    end
    if (get(handles.dz2,'Value') == 1); j0 = [j0 7];
        y1 = y1 + dz2_up; y2 = y2 + dz2_dn;
    end
    if (get(handles.dx2,'Value') == 1); j0 = [j0 1]; 
        up = cosd(120)*dz2_up + sind(120)*dx2y2_up;
        for i = 1:length(up); if up(i) < 0; up(i) = 0; end; end
        if size(dos,2) == 2; 
            dn = cosd(120)*dz2_dn + sind(120)*dx2y2_dn; 
            for i = 1:length(dn); if dn(i) < 0; dn(i) = 0; end; end
        end
        y1 = y1 + up; y2 = y2 + dn;
    end
    if (get(handles.dy2,'Value') == 1); j0 = [j0 2]; 
        up = cosd(60)*dz2_up + sind(60)*dx2y2_up;
        for i = 1:length(up); if up(i) < 0; up(i) = 0; end; end
        if size(dos,2) == 2; 
            dn = cosd(60)*dz2_dn + sind(60)*dx2y2_dn; 
            for i = 1:length(dn); if dn(i) < 0; dn(i) = 0; end; end
        end
        y1 = y1 + up; y2 = y2 + dn;
    end
    if (get(handles.dz2x2,'Value') == 1); j0 = [j0 3]; 
        up = cosd(150)*dz2_up + sind(150)*dx2y2_up;
        for i = 1:length(up); if up(i) < 0; up(i) = 0; end; end
        if size(dos,2) == 2; 
            dn = cosd(150)*dz2_dn + sind(150)*dx2y2_dn; 
            for i = 1:length(dn); if dn(i) < 0; dn(i) = 0; end; end
        end
        y1 = y1 + up; y2 = y2 + dn;  
    end
    if (get(handles.dy2z2,'Value') == 1); j0 = [j0 4]; 
        up = cosd(30)*dz2_up + sind(30)*dx2y2_up;
        for i = 1:length(up); if up(i) < 0; up(i) = 0; end; end
        if size(dos,2) == 2; 
            dn = cosd(30)*dz2_dn + sind(30)*dx2y2_dn; 
            for i = 1:length(dn); if dn(i) < 0; dn(i) = 0; end; end
        end
        y1 = y1 + up; y2 = y2 + dn;  
    end
...
elseif opt_dos == 3 
    ...
    elseif get(handles.cek_pband,'Value') == 1 % PBANDS
        ...
        else pband1 = procar_lnon(pathname); pband2 = []; end
        dx2y2_up = pband1(:,:,:,9); dz2_up = pband1(:,:,:,7); 
        pband1(:,:,:,1) = cosd(120)*dz2_up + sind(120)*dx2y2_up;
        pband1(:,:,:,2) = cosd(60)*dz2_up + sind(60)*dx2y2_up;
        pband1(:,:,:,3) = cosd(150)*dz2_up + sind(150)*dx2y2_up;
        pband1(:,:,:,4) = cosd(30)*dz2_up + sind(30)*dx2y2_up;
        if ~isempty(pband2);
            dx2y2_up = pband2(:,:,:,9); dz2_up = pband2(:,:,:,7); 
            pband2(:,:,:,1) = cosd(120)*dz2_up + sind(120)*dx2y2_up;
            pband2(:,:,:,2) = cosd(60)*dz2_up + sind(60)*dx2y2_up;
            pband2(:,:,:,3) = cosd(150)*dz2_up + sind(150)*dx2y2_up;
            pband2(:,:,:,4) = cosd(30)*dz2_up + sind(30)*dx2y2_up;
        end
        ...
        if size(eig,3) == 2
            subplot(1,2,1)
            plt_pband(kpoint,eig(:,:,2),pband1,ion,j0,lcol,mcol,lwidth,Msize,ax)
            ylabel('E-E_F (eV)','FontSize',18); subplot(1,2,2)
            plt_pband(kpoint,eig(:,:,1),pband2,ion,j0,lcol,mcol,lwidth,Msize,ax)
        else ylabel('E-E_F (eV)','FontSize',18)
            plt_pband(kpoint,eig(:,:,1),pband1,ion,j0,lcol,mcol,lwidth,Msize,ax)
        end
    end
elseif opt_dos == 4 
...
xlabel('DOS','FontSize',18); axis(ax);
...
function axa_Callback(~, ~, handles)
set(handles.axc,'Value',0); set(handles.axc,'ForegroundColor',[0 0 0])
set(handles.axb,'Value',0); set(handles.axb,'ForegroundColor',[0 0 0])
set(handles.axa,'Value',1); set(handles.axa,'ForegroundColor',[0 0 1])
set(handles.dxy,'String','d_yz'); set(handles.dx2,'String','d_3y2-r2');
set(handles.dyz,'String','d_xz'); set(handles.dy2,'String','d_3z2-r2');
set(handles.dxz,'String','d_xy'); set(handles.dz2,'String','d_3x2-r2');
set(handles.dx2y2,'String','d_y2-z2'); set(handles.dy2z2,'String','d_x2-z2');
set(handles.dz2x2,'String','d_x2-y2');

function axb_Callback(~, ~, handles)
set(handles.axc,'Value',0); set(handles.axc,'ForegroundColor',[0 0 0])
set(handles.axa,'Value',0); set(handles.axa,'ForegroundColor',[0 0 0])
set(handles.axb,'Value',1); set(handles.axb,'ForegroundColor',[0 0 1])
set(handles.dxy,'String','d_xz'); set(handles.dx2,'String','d_3z2-r2');
set(handles.dyz,'String','d_xy'); set(handles.dy2,'String','d_3x2-r2');
set(handles.dxz,'String','d_yz'); set(handles.dz2,'String','d_3y2-r2');
set(handles.dx2y2,'String','d_x2-z2'); set(handles.dy2z2,'String','d_x2-y2');
set(handles.dz2x2,'String','d_y2-z2');

function axc_Callback(~, ~, handles)
set(handles.axa,'Value',0); set(handles.axa,'ForegroundColor',[0 0 0])
set(handles.axb,'Value',0); set(handles.axb,'ForegroundColor',[0 0 0])
set(handles.axc,'Value',1); set(handles.axc,'ForegroundColor',[0 0 1])
set(handles.dxy,'String','d_xy'); set(handles.dx2,'String','d_3x2-r2');
set(handles.dyz,'String','d_yz'); set(handles.dy2,'String','d_3y2-r2');
set(handles.dxz,'String','d_xz'); set(handles.dz2,'String','d_3z2-r2');
set(handles.dx2y2,'String','d_x2-y2'); set(handles.dy2z2,'String','d_y2-z2');
set(handles.dz2x2,'String','d_x2-z2');
...
function dxy_Callback(~, ~, handles)
set(handles.cek_all,'Value',0)

function dyz_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dxz_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dx2y2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dy2z2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dz2x2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dx2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dy2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function dz2_Callback(~, ~, handles)
set(handles.cek_all,'Value',0); set(handles.cek_all,'ForegroundColor',[0 0 0])

function cek_pband_Callback(~, ~, handles)
if get(handles.cek_pband,'Value') == 1
    set(handles.cek_pband,'ForegroundColor',[0 0 1])
else set(handles.cek_pband,'ForegroundColor',[0 0 0])
end

function cek_all_Callback(~, ~, handles)
if get(handles.cek_all,'Value') == 1
    set(handles.dxy,'Value',1);   set(handles.dyz,'Value',1)
    set(handles.dxz,'Value',1);   set(handles.dy2,'Value',1)
    set(handles.dx2y2,'Value',1); set(handles.dz2x2,'Value',1)
    set(handles.dy2z2,'Value',1); set(handles.dx2,'Value',1)
    set(handles.dz2,'Value',1);
    set(handles.cek_all,'ForegroundColor',[0 0 1])
else
    set(handles.dxy,'Value',0);   set(handles.dyz,'Value',0)
    set(handles.dxz,'Value',0);   set(handles.dy2,'Value',0)
    set(handles.dx2y2,'Value',0); set(handles.dz2x2,'Value',0)
    set(handles.dy2z2,'Value',0); set(handles.dx2,'Value',0)
    set(handles.dz2,'Value',0);
    set(handles.cek_all,'ForegroundColor',[0 0 0])
end
...
if numel(buffer) ~=0
    nedos = buffer(3); efermi = buffer(4); 
    ...
    if max(size(buffer)) <= 5
        ...
        if fgetl(fid)~=-1
            buffer = sscanf(fgetl(fid),'%f');
            orb = max(size(buffer)); fseek(fid, position, 'bof');
            pdos = zeros([nedos orb-1 natoms]);
            for i=1:natoms
                if fgetl(fid)~=-1
...
fid = fopen([pathname 'EIGENVAL']);
buffer = fscanf(fid, '%d', 4); ispin = buffer(4); fgetl(fid); fgetl(fid); 
fgetl(fid); fgetl(fid); fgetl(fid); buffer = fscanf(fid, '%d', 3); 
nkpoints = buffer(2); nbands = buffer(3); fclose(fid);
fid = fopen([pathname 'DOSCAR']);
buffer = fscanf(fid, '%f', 4)'; natoms = buffer(1); 
fgetl(fid); fgetl(fid); fgetl(fid); fgetl(fid); fgetl(fid); 
...
fclose(fid); pband1 = zeros(nkpoints, nbands, natoms, orbital);
...
buffer2 = sscanf(fgetl(fid),'%f');
orb = max(size(buffer)); orb2 = max(size(buffer2));
if orb == orb2; orbital = 9; else orbital = 16; end
fclose(fid); pband = zeros(nkpoints, nbands, natoms, orbital);
fid = fopen([pathname 'PROCAR']);
opt = fgetl(fid); opt = length(opt); fgetl(fid); fgetl(fid);
fgetl(fid); fgetl(fid); fgetl(fid); fgetl(fid); fgetl(fid); 
for i = 1:nkpoints
